...
_parse_stream_attr(BITSTREAM *bits, CLPI_PROG_STREAM *ss)
{
    ...
    ss->lang[3] = '\0';
    // BD_DEBUG(DBG_NAV | DBG_CRIT, "pid(%x) coding_type(%x) format(%x) rate(%d) aspect(%d) oc_flag(%d) char_code(%d) lang(%s)\n",
    //         ss->pid, ss->coding_type, ss->format, ss->rate, ss->aspect, ss->oc_flag, ss->char_code, ss->lang);
...
_parse_program(BITSTREAM *bits, CLPI_PROG_INFO *program)
{
    ...
    program->progs = progs;
    // BD_DEBUG(DBG_NAV | DBG_CRIT, "num_prog(%d)\n", program->num_prog);
    for (ii = 0; ii < program->num_prog; ii++) {
        ...
        progs[ii].streams = ps;
        // BD_DEBUG(DBG_NAV | DBG_CRIT, "progs[%d] num_streams(%d)\n", ii, progs[ii].num_streams);
        for (jj = 0; jj < progs[ii].num_streams; jj++) {
            ps[jj].pid = bs_read(bits, 16);
            // BD_DEBUG(DBG_NAV | DBG_CRIT, "progs[%d] streams[%d] pid(%x)\n", ii, jj, ps[jj].pid);
