                                       ...
                                       uint32_t *lang, uint32_t blacklist)
{
    ...
    /* BD_DEBUG(DBG_BLURAY, "_update_stream_psr_by_lang: psr_lang(%x)\n", psr_lang);
    BD_DEBUG(DBG_BLURAY, "_update_stream_psr_by_lang: psr_stream(%x)\n", psr_stream);
    BD_DEBUG(DBG_BLURAY, "_update_stream_psr_by_lang: enable_flag(%x)\n", enable_flag);
    BD_DEBUG(DBG_BLURAY, "_update_stream_psr_by_lang: num_streams(%x)\n", num_streams);
    BD_DEBUG(DBG_BLURAY, "_update_stream_psr_by_lang: blacklist(%x)\n", blacklist);
    BD_DEBUG(DBG_BLURAY, "_update_stream_psr_by_lang: preferred_lang(%x)\n", preferred_lang);
    for (ii = 0; ii < num_streams; ii++) {
        uint32_t lang = str_to_uint32((const char *)streams[ii].lang, 3);
        BD_DEBUG(DBG_BLURAY, "_update_stream_psr_by_lang: lang[%s](%x)\n", streams[ii].lang, lang);
    } */
	// dump_backtrace("libbluray", 1);
...
char *bd_stream_path(BLURAY *bd)
{
    char *filepath = str_printf("%s%s%s", disc_root(bd->disc), "BDMV" DIR_SEP "STREAM" DIR_SEP, bd->st0.clip->name);
    char *path = str_dup(filepath);
...
{
    if (bd->title && bd->st0.clip && bd->st0.clip->ref < bd->title->clip_list.count) {
        NAV_CLIP *clip = &bd->title->clip_list.clip[bd->st0.clip->ref];
        return clpi_copy(clip->cl);
    }
    return NULL;
}

struct nav_mark_list_s *bd_get_chap(BLURAY *bd)
{
    if (bd->title) {
        return &bd->title->chap_list;
    }
    return NULL;
}
